...

{
  config = {
    wayland.windowManager.hyprland = with lib;
      let
        plugin_path = (p: (toString p) + "/lib/lib" + p.pname + ".so");
        load_plugins = (plugins:
          concatStringsSep "\n"
          (map (path: "exec-once = hyprctl plugin load " + path)
            (map plugin_path plugins)));
      ...
      in {
        ...
        extraConfig = ''
          # This is needed to fix weird crashing issues when loading plugins the traditional way 
          ${load_plugins plugins}
          ...
          bind = , XF86AudioRaiseVolume, exec, ${pkgs.swayosd}/bin/swayosd --output-volume 5
          bind = , XF86AudioLowerVolume, exec, ${pkgs.swayosd}/bin/swayosd --output-volume -5
