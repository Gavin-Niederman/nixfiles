...
export class NiriService extends Service {
    ...
    get active_windows() {
        return Object.values(this._activeWindows)
            .map((id) => this.getWindow(id))
            .filter((w) => w !== undefined) as Window[];
    }
    ...
    get focused_window() {
        return this._focusedWindow !== undefined
            ? this.getWindow(this._focusedWindow)
            : undefined;
    }

    readonly getWorkspace = (id: number) =>
        this._workspaces.find((w) => w.id === id);
    ...
    private readonly handleEvent = (event: Event) => {
        if ("WorkspacesChanged" in event) {
            ...
            this._activeWorkspaces.filter(
                (w) =>
                    this.getWorkspace(w)?.output !== workspace.output ?? true,
            );
            ...
            console.warn(
                `Unknown niri event variant: ${Object.keys(event)[0]}`,
            );
        }
    };
    ...
    constructor() {
        ...
        if (GLib.getenv("NIRI_SOCKET") === null) {
            logError(
                "Niri IPC is not running. The Niri service will not work properly.",
            );
        }
        ...
        this._activeWorkspaces = this.workspaces
