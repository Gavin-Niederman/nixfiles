...
function colorsScss(flavor: "macchiato" | "latte" | "frappe" | "mocha") {
    ...
    return Object.keys(colors).map(key => `$${key}: ${colors[key]};`).join("\n");
}
function variablsScss(flavor: "macchiato" | "latte" | "frappe" | "mocha") {
    return colorsScss(flavor) + `
        $corner-radius: 16px;
        $ui-radius: 8px;
        $transition: all 0.2s;
    `
...
class StyleService extends Service {
    ...
    async buildStyle() {
        ...
        const scssFiles = (await Utils.execAsync(`fd .scss ${App.configDir}`)).split(/\n+/);
        const imports = ["/tmp/ags/variables.scss", ...scssFiles].map(file => `@import "${file}";`).join("\n");

        await Utils.writeFile(variables, "/tmp/ags/variables.scss");
        await Utils.writeFile(imports, "/tmp/ags/style.scss");

        await Utils.execAsync([
            "sass", `/tmp/ags/style.scss`, this.outputPath
        ]);
