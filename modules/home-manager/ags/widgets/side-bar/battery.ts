...
const BatteryIcon = (icon: string) =>
    SymbolicIcon({
        icon,
        size: 24,
    });

const Battery = (vertical: boolean) =>
    Widget.Box({
        vertical,
        className: "battery",
        children: [
            ...
            Widget.Label({
                ...
                label: BatteryService.bind("percent").as(
                    (p) => `${p.toFixed(0)}%`,
                ),
            }),
            Widget.LevelBar({
                ...
                classNames: Utils.merge(
                    [
                        BatteryService.bind("percent").as((p) =>
                            ["battery-bar"]
                    ...
                    (level, charging) =>
                        charging ? level.concat("charging") : level,
                ),
